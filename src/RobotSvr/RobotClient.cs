...
namespace RobotSvr
{
    ...
    public class RobotClient
    {
        ...
        public void ActorCheckHealth(bool bNeedSP)
        {
            ...
            usidx = -1;
            ubidx = -1;
            MaxHP = int.MaxValue / 2 - 1;
            MaxMP = int.MaxValue / 2 - 1;
            MaxSP = int.MaxValue / 2 - 1;
        ...
        public bool AttackTarget(TActor target)
        {
            ...
            if ((Math.Abs(MShare.g_MySelf.m_nCurrX - target.m_nCurrX) <= 1) && (Math.Abs(MShare.g_MySelf.m_nCurrY - target.m_nCurrY) <= 1) && (!target.m_boDeath))
            {
                if (TimerAutoPlay.Enabled)
                {
                    ...
                    {
                        MShare.g_sAPStr = string.Format("怪物目标：{0} ({1},{2}) 正在使用普通攻击", MShare.g_APTagget.m_sUserName, MShare.g_APTagget.m_nCurrX, MShare.g_APTagget.m_nCurrY);
                    }
                }
            lab:
            ...
            else
            {
                ...
                if (TimerAutoPlay.Enabled)
                {
                    ...
                    if (target != null)
                    {
                        MShare.g_sAPStr = string.Format("怪物目标：{0} ({1},{2}) 正在跑向", target.m_sUserName, target.m_nCurrX, target.m_nCurrY);
                    }
                }
            }
            return result;
        ...
        public void MouseTimerTimer(object Sender, System.EventArgs _e1)
        {
            int i;
            int ii;
            int fixidx;
                    ...
                    for (i = MShare.g_UseItems.GetUpperBound(0); i >= MShare.g_UseItems.GetLowerBound(0); i--)
                    {
                        if (MShare.g_UseItems[i].Item.Name != "")
                        {
                            ...
                            if (MShare.g_UseItems[i].Dura < 1500)
                            {
                                ...
                                {
                                    DScreen.AddSysMsgCenter(string.Format("你的[%s]持久已到底限，请及时修理！", new string[] { MShare.g_UseItems[i].Item.Name }), Color.Lime, Color.Black, 10);
                                }
                                if (MShare.g_gcGeneral[9])
                                {
                                    ...
                                    {
                                        DScreen.AddSysMsgCenter(string.Format("你的修复神水已经用完，请及时补充！", new string[] { MShare.g_UseItems[i].Item.Name }), Color.Lime, Color.Black, 10);
                                    }
                                }
                            }
                        }
        ...
        public void WaitMsgTimerTimer(object Sender, System.EventArgs _e1)
        {
            if (MShare.g_MySelf == null)
            {
                return;
        ...
        public void SelChrWaitTimerTimer(object Sender, System.EventArgs _e1)
        {
        ...
        public void CmdTimerTimer(object Sender, System.EventArgs _e1)
        {
        ...
        public void SendSocket(string sendstr)
        {
            ...
            {
                ClientSocket.SendText(string.Format("#1{0}!", sendstr));
            }
        }
        ...
        public void SendRunLogin()
        {
            ...
            //    Dest = string.Format("%1.2x", new int[] { offset });
            ...
            //        Dest = Dest + string.Format("%1.2x", new int[] { SrcAsc });
            //        offset = SrcAsc;
            //    }
            //}
            //catch
            //{
            //}
            //sSendMsg = string.Format("**%s/%s/%d/%d/%d/%s", new string[] { LoginID, m_sCharName, Certification, MShare.CLIENT_VERSION_NUMBER, MShare.RUNLOGINCODE, Dest });
                ...
                if (HUtil32.CompareLStr(rstr, sam, sam.Length))
                {
                    ...
                    if (param != "")
                    {
                        ...
                        if ((sx != "") && (sy != ""))
                        {
                            ...
                            {
                                // 自动移动
                                DScreen.AddChatBoardString(string.Format("到达 %s 之后才能使用自动走路", new string[] { sM }), Color.Blue, Color.White);
                                return;
                            }
                            X = Convert.ToInt32(sx);
                            Y = Convert.ToInt32(sy);
        ...
        public bool IsUnLockAction()
        {
            ...
            {
                // 如果操作被锁定，则在指定时间后解锁
                if ((MShare.GetTickCount() - ActionFailLockTime) >= 1000)
                {
                    ActionFailLock = false;
                }
            }
                ...
                if (!g_PathBusy)
                {
                    ...
                    try
                    {
                        ...
                        else
                        {
                            ...
                            MShare.g_MySelf.m_nTagY = 0;
                            TimerAutoMove.Enabled = false;
                            DScreen.AddChatBoardString(string.Format("自动移动目标(%d:%d)被占据，不可到达", new int[] { MShare.g_MySelf.m_nTagX, MShare.g_MySelf.m_nTagY }), GetRGB(5), Color.White);
                        }
                    }
                    finally
                    {
        ...
        private void DecodeMessagePacket(string datablock, int btPacket)
        {
            ...
            switch (msg.Ident)
            {
                ...
                case Grobal2.SM_ABILITY:
                    ...
                    EDcode.DecodeBuffer(body, ref MShare.g_MySelf.m_Abil);
                    break;
                ...
                case Grobal2.SM_WINEXP:
                    ...
                    {
                        DScreen.AddSysMsgBottom(string.Format("经验值 +{0}", HUtil32.MakeLong(msg.Param, msg.Tag)));
                    }
                    break;
        ...
        private void ClientGetNeedUpdateAccount(string body)
        {
            ...
            EDcode.DecodeBuffer(body, ref ue);
        ...
        private void ClientGetAddItem(int Hint, string body)
        {
            ...
            {
                cu = new TClientItem();
                EDcode.DecodeBuffer(body, ref cu);
        ...
        private void ClientGetSendUseItems(string body)
        {
            ...
            while (true)
            {
                ...
                {
                    cu = new TClientItem();
                    EDcode.DecodeBuffer(data, ref cu);
        ...
        private void ClientGetAddMagic(string body)
        {
            ...
            EDcode.DecodeBuffer(body, ref pcm);
            MShare.g_MagicArr[0][pcm.Def.wMagicID] = pcm;
        ...
        private void ClientGetMyMagics(string body)
        {
            ...
            MShare.g_MagicList.Clear();
            for (var i = 0; i < MShare.g_IPMagicList.Count; i++)
            {
                //this.Dispose((TClientMagic)MShare.g_IPMagicList[i]);
            }
            MShare.g_IPMagicList.Clear();
            while (true)
            {
                ...
                {
                    pcm = null;
                    EDcode.DecodeBuffer(data, ref pcm);
                    MShare.g_MagicList.Add(pcm);
                    MShare.g_MagicArr[0][pcm.Def.wMagicID] = pcm;
                }
                else
        ...
        private void ClientGetMagicLvExp(int magid, int maglv, int magtrain)
        {
            ...
            TClientMagic pcm = MShare.g_MagicArr[nType][magid];
            if (pcm != null)
            {
        ...
        private void ClientGetMagicMaxLv(int magid, int magMaxlv, int hero)
        {
            ...
            if (hero == 0)
            {
                TClientMagic pcm = MShare.g_MagicArr[0][magid];
        ...
        public void MinTimerTimer(object Sender, System.EventArgs _e1)
        {
            int i;
        ...
        private void CmdShowHumanMsg(string sParam1, string sParam2, string sParam3, string sParam4, string sParam5)
        {
            ...
            //    {
            //        DScreen.AddChatBoardString(string.Format("%s没找到", new string[] { sHumanName }), Color.White, Color.Red);
            //        return;
            //    }
            //}
            //g_boShowMemoLog = !g_boShowMemoLog;
        ...
        private void ProcessActMsg(string datablock)
        {
            ...
            if ((datablock[2] == 'G') && (datablock[3] == 'D') && (datablock[4] == '/'))
            {
                ...
                if (data != "")
                {
                    rtime = HUtil32.Str_ToInt(data, 0);
                    if (rtime <= 0)
                    {
                        return;
                    }
