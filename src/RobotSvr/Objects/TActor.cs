...
public class TActor
{
    ...
    public TUseMagicInfo m_CurMagic;
    ...
    public void SendMsg(int wIdent, int nX, int nY, int ndir, int nFeature, int nState, string sStr, int nSound,
        int dwDelay = 0)
    {
        TChrMsg Msg;
        Msg = new TChrMsg();
        ...
        if (!m_boDeath)
        {
            ...
            if (MShare.g_MySelf == this)
            {
                ...
                if (Msg.Ident == Grobal2.CM_RUN)
                    if (!robotClient.g_PlayScene.CanRun(MShare.g_MySelf.m_nCurrX, MShare.g_MySelf.m_nCurrY, Msg.X,
                            Msg.Y))
                        return;
