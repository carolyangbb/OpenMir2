using MirClient.MirSounds.Libraries;
using SharpDX.DirectSound;
using System.Collections.Generic;
using System.IO;
using System.Linq;

namespace MirClient.MirSounds
{
    static class SoundManager
    {
        ...
        public static void ProcessDelayedSounds()
        {
            ...
            var sounds = DelayList.Where(x => x.Key <= GameFrm.Time).ToList();
        ...
        public static void PlaySound(int index, bool loop = false, int delay = 0)
        {
            if (delay > 0)
            {
                DelayList.Add(new KeyValuePair<long, int>(GameFrm.Time + delay, index));
                return;
            }
        ...
        {
            if (GameFrm.Time >= _checkSoundTime)
            {
                _checkSoundTime = GameFrm.Time + 30 * 1000;
                ...
                for (int i = Sounds.Count - 1; i >= 0; i--)
                {
                    ...
                    if (!sound.IsPlaying())
                    {
                        if (GameFrm.Time >= sound.ExpireTime)
                        {
                            sound.Dispose();
                            Sounds.RemoveAt(i);
                            continue;
